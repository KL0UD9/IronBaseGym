...
interface ClassWithBookings {
  ...
  bookings: Array<{ id: string; user_id: string }>;
}
...
export default function BookClassPage() {
  ...
  const fetchClasses = async () => {
    ...
    const weekEnd = addDays(weekStart, 14); // Show 2 weeks
    ...
    if (error) {
      ...
      setClasses((data as ClassWithBookings[]) || []);
    }
    setLoading(false);
  };
  ...
  const handleBookClass = async (classId: string) => {
    ...
    const confirmedBookings = classItem.bookings.filter(b => b.user_id).length;
    ...
    if (error) {
      ...
      toast.success(isFull ? 'Added to waitlist!' : 'Class booked successfully!');
      fetchClasses();
    }
  ...
  const isUserBooked = (classItem: ClassWithBookings) => {
    return classItem.bookings.some(b => b.user_id === user?.id);
  };

  const getAvailableSpots = (classItem: ClassWithBookings) => {
    const booked = classItem.bookings.length;
    return Math.max(0, classItem.capacity - booked);
  };
                      ...
                      >
                        ...
                        <div className="space-y-2 mb-4">
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            {format(new Date(classItem.start_time), 'h:mm a')} • {classItem.duration_min} min
                          </div>
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {classItem.trainer && (
                            <p className="text-sm text-primary">
                              with {classItem.trainer.full_name}
                            </p>
                          )}
                        </div>
                        ...
                        {!isBooked && (
                          <Button
                            ...
                            disabled={bookingInProgress === classItem.id}
